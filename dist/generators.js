...
"use strict";
var __moduleName = "src_generators";
...
function* inorder(node) {
  ...
  while (true) {
    ...
    while (true) {
      var childGeneratorResult = childGenerator.next();
      if (childGeneratorResult.done) {
        if (yieldedValue && childGeneratorResult.done)
          return childGeneratorResult.value;
        yield childGeneratorResult.value;
        break;
      } else {
...
function* breadthFirst(node) {
  ...
  var twoPass = repeat(transform(node, (function(value) {
    return breadthFirst(value);
  }), 2));
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (passResult.done && childResult.done)
...
function toNode(generator) {
  ...
  if (!result.done)
    ret.children = toArray(generator);
  return ret;
}
function asNode(generator) {
  ...
  return toNode(generator.apply(null, $traceurRuntime.spread(args)));
}
